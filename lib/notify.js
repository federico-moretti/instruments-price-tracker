...
function shouldNotifyInChangesMode(item) {
  if (item.type === 'error') {
    return true;
  }
...
function filterResultsForMode(results, mode) {
  if (mode === 'full') {
    return results;
  }

  return results.filter(shouldNotifyInChangesMode);
}

export async function notifyCheckSummary({ wishlistName, results, mode = 'full' }) {
  const filteredResults = filterResultsForMode(results, mode);

  if (filteredResults.length === 0) {
    console.log(`No changes for "${wishlistName ?? 'wishlist'}"; skipping notification`);
    return;
  }

  const lines = filteredResults.map(formatCheckLine);
